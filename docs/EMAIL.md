...
- Transactional email: magic links, estimate links, change order approvals, payment receipts, and job updates. These must be sent immediately from the API and currently use Resend.
...
Use Resend for transactional and operational email. It is the active provider for Crewmodo because `mail.crewmodo.com` is verified for sending and the Worker already has a Resend adapter.
...
Cloudflare Email Service is still a possible future migration path if we want to remove external dependencies. Do not switch production to Cloudflare Email Service until sender DNS, test delivery, bounce handling, and operational observability are verified from staging.
...
Required DNS records for `mail.crewmodo.com`:
...
Resend reports `mail.crewmodo.com` as verified with sending enabled.
...
The app only sends `RESEND_API_KEY` to Resend. SPF, DKIM, DMARC, and provider verification belong in DNS/provider setup, not in the application payload.
